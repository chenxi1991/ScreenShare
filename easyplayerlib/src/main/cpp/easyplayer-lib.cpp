...
void showPic() {
    ...
    if (0 > ANativeWindow_setBuffersGeometry(nativeWindow, mPlayer->viddec.get_width(),
                                             mPlayer->viddec.get_height(),
    ...
    int numBytes = av_image_get_buffer_size(AV_PIX_FMT_RGBA, mPlayer->viddec.get_width(),
                                            mPlayer->viddec.get_height(), 1);
    ...
    av_image_fill_arrays(frameRGBA->data, frameRGBA->linesize, vOutBuffer, AV_PIX_FMT_RGBA,
                         mPlayer->viddec.get_width(), mPlayer->viddec.get_height(), 1);
...
//    player.init(inputStr);
    ...
    if (easyPlayer.has_video()) {
        env->CallVoidMethod(obj, gOnResolutionChange, easyPlayer.viddec.get_width(),
                            easyPlayer.viddec.get_height());
    }
